...
unsigned char RX0_Address[]={ 0xA6, 0x0B, 0x38, 0x7C, 0x2B };
...
void rfm70_buffer_write( 
   ...
   MR_CSN( 0 );                     // Set CSN low, init SPI tranaction
...
void rfm70_receive_address_p0( unsigned char *address ){
   rfm70_buffer_write( RFM70_REG_RX_ADDR_P0, address, 5 );  
}

void rfm70_receive_address_p1( unsigned char *address ){
...
void rfm70_transmit_address( unsigned char *address ){
...
void rfm70_init( void ){
   ...
   i=rfm70_register_read(RFM70_REG_CONFIG);
   i&=(~(0b1<<6))&(~(0b1<<5)); //Active IRQ TX et RX
   i|=0b1<<4; //Désactive IRQ MAX_RT
   rfm70_register_write(RFM70_REG_CONFIG,i);
...
{
   rfm70_channel(0);
   rfm70_air_data_rate(2);
   rfm70_lna_high();
   rfm70_power(3);
   rfm70_retransmit_delay_attempts(3,4);
}


//RFM70_REG_CONFIG
//! - 6 : 1 masks RX_DR (see...) from IRQ pin, 0 allows
//! - 5 : 1 masks TX_DS (see...) from IRQ pin, 0 allows
//! - 4 : 1 masks MAX_RT (see...) from IRQ pin, 0 allows
void enable_TX_IRQ()
{
   unsigned char res=rfm70_register_read(RFM70_REG_CONFIG);
   rfm70_register_write(RFM70_REG_CONFIG,res&(~(0b1<<5)));
}
...
{
   unsigned char res=rfm70_register_read(RFM70_REG_CONFIG);
   rfm70_register_write(RFM70_REG_CONFIG,res&(~(0b1<<6)));
}
...
//#define ST_RG_DATA_NRDY (0b1<<6)
//#define ST_RG_DATA_NRDY (0b1<<5)
unsigned char clear_IRQ()//return the IRQ sources
{
   unsigned char res=rfm70_register_read(RFM70_REG_STATUS);
   rfm70_register_write(RFM70_REG_STATUS,res|0b01110000);
   return res;
}


unsigned char get_IRQ_RX()//Return 0xFF si data received
{
   unsigned char res=rfm70_register_read(RFM70_REG_STATUS);
   if(res&(0b1<<6))return 0x00;
   else
   {
      //rfm70_register_write(RFM70_REG_STATUS,res|(0b1<<6));
      return 0xFF;
   }
}
unsigned char get_IRQ_TX()//Return 0xFF si data transmited
{
   unsigned char res=rfm70_register_read(RFM70_REG_STATUS);
   if(res&(0b1<<5))return 0xFF;
   else
   {
      //rfm70_register_write(RFM70_REG_STATUS,res|(0b1<<5));
      return 0x00;
   }
}
...
{
   unsigned char res=rfm70_register_read(RFM70_REG_CONFIG);
   res|=(0b1<<6)|(0b1<<4);//disable MAX et RX
   res&=~(0b1<<5);//enable TX
   rfm70_register_write(RFM70_REG_CONFIG,res);
}

void set_IRQ_to_RX()
{
   unsigned char res=rfm70_register_read(RFM70_REG_CONFIG);
   res|=(0b1<<5)|(0b1<<4);//disable MAX et TX
   res&=~(0b1<<6);//enable RX
   rfm70_register_write(RFM70_REG_CONFIG,res);
}
